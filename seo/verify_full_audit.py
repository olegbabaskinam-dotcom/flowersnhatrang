...
def main() -> int:
    ...
    for row in products:
        ...
        expected = math.ceil((vnd / 265) / 100) * 100
        rub_match = re.search(r"(\d[\d ]*) ₽$", row["price_sub"])
        actual = int(re.sub(r"\D", "", rub_match.group(1))) if rub_match else -1
        if actual != expected:
            errors.append(f"курс 265 нарушен в products.csv: ID {row['id']} — {actual}, ожидается {expected}")
...
"""
    ...
    print(f"Цены проверены по курсу 265: {len(products)}")
